...
export function ChatProvider({ children }) {
  ...
  const fetchMessages = async (email) => {
    ...
    try {
      ...
      const response = await fetch(`https://chat-7-s9jw.onrender.com/api/messages/${encodeURIComponent(email)}`, {
        credentials: 'include',
      });
  ...
  const deleteMessage = async (messageId) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`https://chat-7-s9jw.onrender.com/api/messages/${messageId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
